...
package {
    ...
    import com.goshare.component.AppLoading;
    ...
    import com.goshare.manager.VersionManager;
    ...
    public class Scratch extends Application {
        ...
        public static var sb2Player: Sb2Player

        public static var fullStageWidth: Number = 1280
        public static var fullStageHeight: Number = 720
        ...
        protected function initGoshareService():void {
            // 全局数据管理器加载配置文件
            AppDataManager.getInstance().init();
            // 轮询工具RunLoop 启动
            AppRunLoopManager.getInstance().init();
            // 核心管理器 初始化
...
//        if (autostart) runtime.startGreenFlags(true);
        ...
        public function fixLayout():void {
            ...
            var h:int = stage.stageHeight; // fix to show bottom border...
        ...
        protected function updateLayout(w:int, h:int):void {
            ...
            if (editMode) {
                if (stageIsContracted) {
                    scaleWidth = 320 / fullStageWidth
                    stagePart.setWidthHeight(320, scaleWidth * fullStageHeight, scaleWidth);
                } else {
                    scaleWidth = 640 / fullStageWidth
                    stagePart.setWidthHeight(640, scaleWidth * fullStageHeight, scaleWidth);
                }
                stagePart.x = w - stagePart.width - 5
                stagePart.y = isMicroworld ? 5 : topBarPart.bottom() + 5 + stagePart.computeTopBarHeight();
                stagePart.scaleY = stagePart.scaleX = 1
                fixLoadProgressLayout();
            } else {
                drawBG();
                var pad:int = (w > 550) ? 16 : 0; // add padding for full-screen mode
                var scale:Number = Math.min((w - pad) / fullStageWidth, (h - pad) / fullStageHeight);
                scale = Math.max(0.01, scale);
                var scaledW:int = Math.floor((scale * fullStageWidth) / 4) * 4; // round down to a multiple of 4
                scale = scaledW / fullStageWidth;
                presentationScale = scale;
                var playerW:Number = (scale * fullStageWidth);
                var playerH:Number = (scale * fullStageHeight);
            ...
            var contentY:int = tabsPart.y + 40;
         ...
         * 恢复为空白项目
         */
        public function clearToBlankProject(callback:Function = null):void
        {
            startNewProject('', '');
