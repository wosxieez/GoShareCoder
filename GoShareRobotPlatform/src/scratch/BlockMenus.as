...
public class BlockMenus implements DragClient {
	...
	public static function BlockMenuHandler(evt:MouseEvent, block:Block, blockArg:BlockArg = null, menuName:String = null):void {
		...
		// Goshare 自定义
		if (menuName == 'openAndClose') menuHandler.openAndCloseMenu(evt);
	}
...
//			handler.scrollAlignMenu(evt);
	...
	private function openAndCloseMenu(evt:MouseEvent):void
	{
		var m:Menu = new Menu(setBlockArg, 'openAndClose');
		m.addItem('打开');
		m.addItem('关闭');
		showMenu(m);
	}
