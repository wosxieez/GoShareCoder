...
public class ScratchCostume {
	...
	public static function scaleForScratch(bm:BitmapData):BitmapData {
		if ((bm.width <= Scratch.fullStageWidth) && (bm.height <= Scratch.fullStageHeight)) return bm;
		var scale:Number = Math.min(Scratch.fullStageWidth / bm.width, Scratch.fullStageHeight / bm.height);
	...
	public function scaleAndCenter(bm:BitmapData, isScene:Boolean):Rectangle {
		var scale:Number = bitmapResolution;
		var costumeBM:BitmapData = bitmapForEditor(isScene);
		var destP:Point = isScene ? new Point(0, 0) :
				new Point(Scratch.fullStageWidth - (scale * rotationCenterX), Scratch.fullStageHeight - (scale * rotationCenterY));
	...
	public static function emptyBitmapCostume(costumeName:String, forBackdrop:Boolean):ScratchCostume {
		var bm:BitmapData = forBackdrop ? new BitmapData(Scratch.fullStageWidth, Scratch.fullStageHeight, true, 0xFFFFFFFF) : new BitmapData(1, 1, true, 0);
	...
	public function thumbnail(w:int, h:int, forStage:Boolean):BitmapData {
		var dispObj:DisplayObject = displayObj();
		var r:Rectangle = forStage ? new Rectangle(0, 0, Scratch.fullStageWidth * bitmapResolution, Scratch.fullStageHeight * bitmapResolution) :
	...
	public function bitmapForEditor(forStage:Boolean):BitmapData {
		...
		if (forStage) {
			w = Scratch.fullStageWidth * bitmapResolution;
			h = Scratch.fullStageHeight * bitmapResolution
		}
