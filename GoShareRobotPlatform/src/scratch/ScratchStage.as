...
package scratch {
    ...
    public class ScratchStage extends ScratchObj {
        ...
        public function ScratchStage() {
            width = Scratch.fullStageWidth
            height = Scratch.fullStageHeight

            objName = 'Stage';
            isStage = true;
            scrollRect = new Rectangle(0, 0, width, height); // clip drawing to my bounds
        ...
        public function countdown(number:int=-1):void {
            ...
            arrowImage.y = 335;
            addChild(arrowImage);
            counter.x = (width-counter.width)/2;
            counter.y = (height-counter.height)/2;
            arrowText.x = 28;
            arrowText.y = 328;
        }
        ...
        private function addPenLayer():void {
            ...
            var bm:BitmapData = new BitmapData(width, height, true, 0);
        ...
        public function scratchMouseX():int { return Math.max(-240, Math.min(mouseX - (width / 2), 240)) }
        public function scratchMouseY():int { return -Math.max(-180, Math.min(mouseY - (height / 2), 180)) }
        ...
        public function saveScreenData():BitmapData {
            var bm:BitmapData = new BitmapData(width,height, false);
...
//				Scratch.app.render3D.getRender(bm);
            ...
            if (Scratch.app !=null && Scratch.app.gh.carriedObj is ScratchSprite) {
                ...
                bm.draw(spr,new Matrix(spr.scaleX*s, 0, 0, spr.scaleY*s, spr.scratchX+(width/2), -spr.scratchY+(height/2)));
            }
        ...
        public function scrollAlign(s:String):void {
            ...
            var sceneW:int = Math.max(c.width, width);
            var sceneH:int = Math.max(c.height, height);
            switch (s) {
                case 'top-left':
                    xScroll = 0;
                    yScroll = sceneH - height;
                    break;
                case 'top-right':
                    xScroll = sceneW - width;
                    yScroll = sceneH - height;
                    break;
                case 'middle':
                    xScroll = Math.floor((sceneW - width) / 2);
                    yScroll = Math.floor((sceneH - height) / 2);
                    break;
                case 'bottom-left':
                    xScroll = 0;
                    yScroll = 0;
        ...
        override protected function updateImage():void {
            ...
            if ((xScroll == 0) && (yScroll == 0) && (c.width == width) && (c.height == height)) {
                img.addChild(currentCostume().displayObj());
                return;
            }

            var bm:BitmapData;
            if ((c is BitmapData) && (c.width >= width) && (c.height >= height)) {
                bm = c as BitmapData;
            } else {
                // render SVG to a bitmap. also centers scenes smaller than the stage
                var m:Matrix = null;
                var insetX:int = Math.max(0, (width - c.width) / 2);
                var insetY:int = Math.max(0, (height - c.height) / 2);
            ...
            if ((xScroll != 0) || (yScroll != 0)) {
                var yBase:int = height - sceneH;
                stageBM = new BitmapData(width, height, false, 0x505050);
...
//	    		testBM.bitmapData = bmd;
        ...
        private function updateCachedBitmap():void {
            ...
            if (!cachedBM) cachedBM = new BitmapData(width, height, false);
