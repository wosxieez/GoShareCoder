...
public class ListWatcher extends Sprite {
	...
	private function importList():void {
		...
		Sb2Player.loadSingleFile(fileLoaded);
	}
	...
	private function importLines(lines:Array):void {
		...
		DialogBox.ask(
			...
			'1', Sb2Player.app.stage, gotColumn);
	}
	...
	public function updateWatcher(i:int, readOnly:Boolean, interp:Interpreter):void {
		// Called by list com.goshare.primitives. Record access to entry at i and whether list contents have changed.
	...
	private function addItem(b:IconButton = null):void {
		...
		if ((root is Sb2Player) && !(root as Sb2Player).editMode) return;
	...
	public function setWidthHeight(w:int, h:int):void {
		// Large (especially tall) list com.goshare.watchers can cause many, many list cells to be allocated, leading to delays.
		// Bound the size so that updateContents() won't cause long delays for long lists.
		var boundingObject:Object = this.parent || Sb2Player.app.stagePane || {width: 480, height: 360};
...
//		if (limitedCloudView &&
//			com.goshare.Sb2Player.app.isLoggedIn() && com.goshare.Sb2Player.app.editMode &&
//			(com.goshare.Sb2Player.app.projectOwner == com.goshare.Sb2Player.app.userName)) {
//				limitedCloudView = false; // only project owner can view cloud list contents
//		}
		var isEditable:Boolean = Sb2Player.app.editMode && !limitedView;
		updateElementCount();
		removeAllCells();
		visibleCells = [];
		visibleCellNums = [];
