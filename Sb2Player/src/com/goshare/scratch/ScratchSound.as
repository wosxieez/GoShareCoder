...
//
// Represents a com.goshare.Sb2Player sampled com.goshare.sound.
// Possible formats:
...
public class ScratchSound {
	...
	public var editorData:Object; // cache of data used by com.goshare.sound editor; not saved
		...
		if (sndData != null) {
			try {
				...
				if (e is Error) {
					...
					Sb2Player.app.log(LogLevel.WARNING, 'Error while constructing com.goshare.sound:' + e.message, extraData);
				}
				else {
					Sb2Player.app.log(LogLevel.WARNING, 'Unknown error while constructing com.goshare.sound', extraData);
				}
	...
	public function convertMP3IfNeeded():void {
		// Support for converting MP3 format sounds in com.goshare.Sb2Player projects was removed during alpha test.
		// If this is on old, MP3 formatted com.goshare.sound, convert it to WAV format. Otherwise, do nothing.
		function whenDone(snd:ScratchSound):void {
			Sb2Player.app.log(LogLevel.INFO, 'Converting MP3 to WAV', {soundName: soundName});
		...
		if (format == 'mp3') {
			if (soundData) {
				...
				Sb2Player.app.log(LogLevel.WARNING, 'No com.goshare.sound data to convert from MP3. Setting empty com.goshare.sound.');
	...
	public function sndplayer():ScratchSoundPlayer {
		...
		if (format == 'squeak') {
			...
			// player on empty com.goshare.sound
	...
	public function getSamples():Vector.<int> {
		...
		Sb2Player.app.log(
				LogLevel.WARNING, 'Unknown com.goshare.sound format in getSamples. Returning empty com.goshare.sound.', {format: format});
	...
	public function prepareToSave():void {
		if (format == 'squeak') { // convert Squeak ADPCM to WAV ADPCM
			...
			Sb2Player.app.log(LogLevel.INFO, 'Converting squeak com.goshare.sound to WAV ADPCM', {
	...
	public function readJSON(jsonObj:Object):void {
		...
		__md5 = jsonObj.md5; // Project load uses this value to fetch com.goshare.sound data
