...
import com.goshare.flvEncoder.*;

import com.goshare.logging.LogLevel;

import com.goshare.primitives.VideoMotionPrims;

import com.goshare.sound.ScratchSoundPlayer;

import com.goshare.translation.*;

import com.goshare.ui.BlockPalette;
import com.goshare.ui.RecordingSpecEditor;
import com.goshare.ui.SharingSpecEditor;
...
public class ScratchRuntime {
	...
	// com.goshare.Sb2Player uses these pseudo-Unicode values to map arrow keys as if they were printable characters.
	...
	public function ScratchRuntime(app:Sb2Player, interp:Interpreter) {
		this.app = app;
		this.interp = interp;
	...
	public function stepRuntime():void {
		...
		if (ready==ReadyLabel.COUNTDOWN) {
			...
			if (tR>=3.75){
				...
				videoSounds = [];
				videoFrames=[];
				if (fullEditor) Sb2Player.app.log(LogLevel.TRACK, "Editor video started",{projectID: app.projectID});
				else Sb2Player.app.log(LogLevel.TRACK, "Project video started",{projectID: app.projectID});
			}
			else if (tR>=2.5){
				count=1
			}
		...
		if (recording) { // Recording a YouTube video?
			...
			if (t>videoSounds.length/videoFramerate+1/videoFramerate) {
				...
				//saves visual frame to frames and com.goshare.sound clip to sounds
				saveFrame();
		...
		// Step the stage, sprites, and com.goshare.watchers
...
//-------- recording video code ---------
	...
	public function saveRecording():void {
		...
		function saveFile():void {
			...
			Sb2Player.app.log(LogLevel.TRACK, "Video downloaded", {projectID: app.projectID, seconds: roundToTens(seconds), megabytes: roundToTens(video.length/1000000)});
			var specEditor:SharingSpecEditor = new SharingSpecEditor();
			DialogBox.close("Playing and Sharing Your Video",null,specEditor,"Back to com.goshare.Sb2Player");
		    releaseVideo(false);
        }
		function releaseVideo(log:Boolean = true):void {
			if (log) Sb2Player.app.log(LogLevel.TRACK, "Video canceled", {projectID: app.projectID, seconds: roundToTens(seconds), megabytes: roundToTens(video.length/1000000)});
            video = null;
		}
...
//			else DialogBox.confirm('Replace contents of the current project?', app.stage, doInstall);
		...
		if (Sb2Player.app.isExtensionDevMode) {
			...
			filter = new FileFilter('com.goshare.Sb2Player Project', '*.sb;*.sb2');
		}
		Sb2Player.loadSingleFile(fileLoadHandler, filter);
	}
...
//		if (app.scriptsPane) app.scriptsPane.viewScriptsFor(null);
		...
		for each (var obj:ScratchObj in project.allObjects()) {
			...
			if(Sb2Player.app.isIn3D) obj.updateCostume();
	...
	public function allSendersOfBroadcast(msg:String):Array {
		// Return an array of all com.goshare.Sb2Player objects that broadcast the given message.
		var result:Array = [];
	...
	public function allReceiversOfBroadcast(msg:String):Array {
		// Return an array of all com.goshare.Sb2Player objects that receive the given message.
		var result:Array = [];
		for each (var o:ScratchObj in app.stagePane.allObjects()) {
	...
	public function allUsesOfSoundDo(soundName:String, f:Function):void {
		for each (var stack:Block in app.viewedObj().scripts) {
			stack.allBlocksDo(function (b:Block):void {
				for each (var a:* in b.args) {
					if (a is BlockArg && a.menuName == 'com.goshare.sound' && a.argValue == soundName) f(a);
				}
			});
		}
	}
	...
	public function allStacksAndOwnersDo(f:Function,setDoObj:Boolean=false):void {
		...
		// This method is used by broadcast, so enumerate sprites/stage from front to back to match com.goshare.Sb2Player.
