...
import flash.utils.*;
	...
	import com.goshare.scratch.ScratchComment;
...
public class ScratchObj extends Sprite {
	...
	public var img:Sprite; // holds a bitmap or svg object, after applying image com.goshare.filters, scale, and rotation
	private var lastCostume:ScratchCostume;

	// Caches used by the com.goshare.interpreter:
	...
	public function deleteCostume(c:ScratchCostume):void {
		...
		if (Sb2Player.app) Sb2Player.app.setSaveNeeded();
	}
	...
	public function unusedSoundName(baseName:String = ''):String {
		// Create a unique com.goshare.sound name by appending a number if necessary.
		if (baseName == '') baseName = 'com.goshare.sound';
		var existingNames:Array = [];
	...
	SCRATCH::allow3d {
		if(this is ScratchStage || this is ScratchSprite || (parent && parent is ScratchStage)) {
			...
			if (Sb2Player.app.isIn3D) Sb2Player.app.render3D.updateRender((this is ScratchStage ? img : this), id, renderOpts);
		}
	}
	}
	...
	public function applyFilters(forDragging:Boolean = false):void {
		...
		clearCachedBitmap();
		if(!Sb2Player.app.isIn3D || forDragging) {
...
//						(this as ScratchStage).updateSpriteEffects((this as ScratchStage).videoImage, filterPack.getAllSettings());
	...
	public function defaultArgsFor(op:String, specDefaults:Array):Array {
		...
		if ((['broadcast:', 'doBroadcastAndWait', 'whenIReceive'].indexOf(op)) > -1) {
			var msgs:Array = Sb2Player.app.runtime.collectBroadcasts();
			return [msgs[0]];
		}
		...
		if ('createCloneOf' == op) {
			...
			sprites = Sb2Player.app.stagePane.sprites();
	...
	public function defaultVarName():String {
		...
		return isStage ? '' : Sb2Player.app.stagePane.defaultVarName(); // global var, if any
	}

	public function defaultListName():String {
		if (lists.length > 0) return lists[lists.length - 1].listName; // local list
		return isStage ? '' : Sb2Player.app.stagePane.defaultListName(); // global list, if any
	}
	...
	public function findSound(arg:*):ScratchSound {
		// Return a com.goshare.sound describe by arg, which can be a string (com.goshare.sound name),
		// a number (com.goshare.sound index), or a string representing a number (com.goshare.sound index).
		...
		if (typeof(arg) == 'number') {
			...
			for each (var snd:ScratchSound in sounds) {
				if (snd.soundName == arg) return snd; // arg matches a com.goshare.sound name
			}
	...
	public function setVarTo(varName:String, value:*):void {
		...
		v.value = value;
		Sb2Player.app.runtime.updateVariable(v);
	}
	...
	public function lookupOrCreateVar(varName:String):Variable {
		...
		if (v == null) { // not found; create it
			...
			variables.push(v);
			Sb2Player.app.updatePalette(false);
		}
		return v;
	}
	...
	public function lookupVar(varName:String):Variable {
		...
		for each (v in Sb2Player.app.stagePane.variables) {
			if (v.name == varName) return v;
		}
		return null;
	}
	...
	public function lookupList(listName:String):ListWatcher {
		...
		for each (list in Sb2Player.app.stagePane.lists) {
			if (list.listName == listName) return list;
		}
		return null;
	}
	...
	public function click(evt:MouseEvent):void {
		var app:Sb2Player = root as Sb2Player;
		if (!app) return;
	...
	public function writeJSON(json:com.goshare.util.JSON):void {
	...
	public function readJSON(jsonObj:Object):void {
		...
		for (var i:int = 0; i < variables.length; i++) {
			...
			variables[i] = Sb2Player.app.runtime.makeVariable(varObj);
		}
		lists = jsonObj.lists || [];
