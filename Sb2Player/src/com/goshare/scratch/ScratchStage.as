...
//
// A com.goshare.Sb2Player stage object. Supports a drawing surface for the pen commands.

package com.goshare.scratch {
...
import com.goshare.filters.FilterPack;
import com.goshare.translation.Translator;
import com.goshare.uiwidgets.Menu;
import com.goshare.ui.media.MediaInfo;
import com.goshare.util.*;
import com.goshare.watchers.*;
import com.goshare.assets.Resources;
...
public class ScratchStage extends ScratchObj {
	...
	public function spriteNamed(spriteName:String):ScratchSprite {
		...
		var app:Sb2Player = Sb2Player.app;
	...
	public function spritesAndClonesNamed(spriteName:String):Array {
		...
		var app:Sb2Player = parent as Sb2Player;
		if (app != null) {
	...
	public function saveScreenData():BitmapData {
		...
		if (SCRATCH::allow3d) {
			if(Sb2Player.app.isIn3D) {
//				com.goshare.Sb2Player.app.render3D.getRender(bm);
			}
			else bm.draw(this);
		}
		else {
	...
	if (Sb2Player.app !=null && Sb2Player.app.gh.carriedObj is ScratchSprite) {
			var spr:ScratchSprite = ScratchSprite(Sb2Player.app.gh.carriedObj);
			var s:Number = 1;
			if (Sb2Player.app.stageIsContracted) {
				s = 2
			}
			if (!Sb2Player.app.editMode) {
				s = 1.0/Sb2Player.app.presentationScale;
			}
	...
	public function getUILayer():Sprite {
		SCRATCH::allow3d {
			if(Sb2Player.app.isIn3D) return Sb2Player.app.render3D.getUIContainer();
		}
		return this;
	}
	...
	override protected function updateImage():void {
		...
		SCRATCH::allow3d {
			if (Sb2Player.app.isIn3D)
				Sb2Player.app.render3D.getUIContainer().transform.matrix = transform.matrix.clone();
		}
	...
	public function step(runtime:ScratchRuntime):void {
		if (videoImage != null) {
			...
			SCRATCH::allow3d { if(Sb2Player.app.isIn3D) Sb2Player.app.render3D.updateRender(videoImage); }
		}
		cachedBitmapIsCurrent = false;

		// Step the com.goshare.watchers
	...
	public function stampSprite(s:ScratchSprite, stampAlpha:Number):void {
		if(s == null) return;
//		if(!testBM.parent) {
//			//testBM.com.goshare.filters = [new GlowFilter(0xFF00FF, 0.8)];
...
//				com.goshare.Sb2Player.app.render3D.updateRender(penLayer);
//	    		testBM.bitmapData = bmd;
	...
	public function getBitmapOfSprite(s:ScratchSprite, bounds:Rectangle, for_carry:Boolean = false):BitmapData {
		...
		if(!Sb2Player.app.render3D || s.width < 1 || s.height < 1) return null;
		...
		var bmd:BitmapData = Sb2Player.app.render3D.getRenderedChild(s, b.width * s.scaleX, b.height * s.scaleY, for_carry);
...
//		Sb2Player.app.libraryPart.showVideoButton();
	...
	public function clearPenStrokes():void {
		...
		penActivity = false;
		SCRATCH::allow3d { if(Sb2Player.app.isIn3D) Sb2Player.app.render3D.updateRender(penLayer); }
	}
	...
	public function bitmapWithoutSprite(s:ScratchSprite):BitmapData {
		// Used by the 'touching color' com.goshare.primitives. Draw the background layers
		// and all sprites (but not com.goshare.watchers or talk bubbles) except the given
...
//	private var dumpPixels:Boolean = true;
	...
	public function updateSpriteEffects(spr:DisplayObject, effects:Object):void {
		if(Sb2Player.app.isIn3D) {
			Sb2Player.app.render3D.updateFilters(spr, effects);
		}
	}
	...
	public function getBitmapWithoutSpriteFilteredByColor(s:ScratchSprite, c:int):BitmapData {
		...
		if (Sb2Player.app.isIn3D) {
			SCRATCH::allow3d {
				bm1 = Sb2Player.app.render3D.getOtherRenderedChildren(s, 1);
			}
		}
		else {
			// OLD code here
...
//		}
	...
	public function updateRender(dispObj:DisplayObject, renderID:String = null, renderOpts:Object = null):void {
		SCRATCH::allow3d {
			if (Sb2Player.app.isIn3D) Sb2Player.app.render3D.updateRender(dispObj, renderID, renderOpts);
		}
	}
	...
	public function projectThumbnailPNG():ByteArray {
		...
		// Get a screenshot of the stage
		if (SCRATCH::allow3d) {
//			if(com.goshare.Sb2Player.app.isIn3D) com.goshare.Sb2Player.app.render3D.getRender(bm);
//			else bm.draw(this);
		}
		else {
			bm.draw(this);
	...
	public function isEmpty():Boolean {
		...
		// at most one sprite, and only the default costumes and com.goshare.sound media.
	...
	public function updateInfo():void {
		...
		if (Sb2Player.app.projectID != '') info.projectID = Sb2Player.app.projectID;
		info.videoOn = isVideoOn();
		info.swfVersion = Sb2Player.versionString;

		delete info.loadInProgress;
		if (Sb2Player.app.loadInProgress) info.loadInProgress = true; // log flag for debugging

		if (this == Sb2Player.app.stagePane) {
			// If this is the active stage pane, record the current com.goshare.extensions.
			var extensionsToSave:Array = Sb2Player.app.extensionManager.extensionsToSave();
			if (extensionsToSave.length == 0) delete info.savedExtensions;
			else info.savedExtensions = extensionsToSave;
		}

		delete info.userAgent;
		if (Sb2Player.app.isOffline) {
			info.userAgent = 'com.goshare.Sb2Player 2.0 Offline Editor';
		}
		else if (Sb2Player.app.jsEnabled) {
			Sb2Player.app.externalCall('window.navigator.userAgent.toString', function(userAgent:String):void {
	...
	public function handleDrop(obj:*):Boolean {
		if ((obj is ScratchSprite) || (obj is Watcher) || (obj is ListWatcher)) {
			...
			if (obj is ScratchSprite) {
				...
				Sb2Player.app.selectSprite(obj);
			...
			if (!(obj is ScratchSprite) || Sb2Player.app.editMode) Sb2Player.app.setSaveNeeded();
			return true;
		}
		Sb2Player.app.setSaveNeeded();
		return false;
	}

	/* Saving */

	public override function writeJSON(json:com.goshare.util.JSON):void {
	...
	public override function readJSON(jsonObj:Object):void {
		...
		// instantiate Watchers and add all children (sprites and com.goshare.watchers)
		for (i = 0; i < children.length; i++) {
			o = children[i];
			if (o is ScratchSprite) {
				...
				if (o.target) {
					if (o.cmd == "senseVideoMotion" && o.param && o.param.indexOf(',')) {
						// fix old video motion/direction com.goshare.watchers
