...
//
// A com.goshare.Sb2Player costume (or scene) is a named image with a rotation center.
...
import com.goshare.svgeditor.objs.SegmentationState;
...
public class ScratchCostume {
	...
	public var oldComposite:BitmapData; // composite bitmap from old com.goshare.Sb2Player file (used only during loading)
	...
	public static function emptyBackdropSVG():ByteArray {
		...
		data.writeUTFBytes(
				...
				'  xmlns="http://www.w3.org/2000/svg" version="1.1"\n' +
				'  xmlns:xlink="http://www.w3.org/1999/xlink">\n' +
				'	<rect x="0" y="0" width="480" height="360" fill="#FFF" com.goshare.scratch-type="backdrop-fill"> </rect>\n' +
				'</svg>\n');
		return data;
	}
	...
	public function setBitmapData(bm:BitmapData, centerX:int, centerY:int):void {
		...
		rotationCenterX = centerX;
		rotationCenterY = centerY;
		if (Sb2Player.app && Sb2Player.app.viewedObj() && (Sb2Player.app.viewedObj().currentCostume() == this)) {
			Sb2Player.app.viewedObj().updateCostume();
			Sb2Player.app.refreshImageTab(true);
		}
	}
	...
	public function setSVGData(data:ByteArray, computeCenter:Boolean, fromEditor:Boolean = true):void {
		...
		function refreshAfterImagesLoaded():void {
			...
			if (Sb2Player.app && Sb2Player.app.viewedObj() && (Sb2Player.app.viewedObj().currentCostume() == thisC)) {
				Sb2Player.app.viewedObj().updateCostume();
				Sb2Player.app.refreshImageTab(fromEditor);
			}
			svgLoading = false;
		}
	...
	public function bitmapForEditor(forStage:Boolean):BitmapData {
		...
		if (SCRATCH::allow3d) {
			...
			Sb2Player.app.ignoreResize = true;
			var oldQuality:String = Sb2Player.app.stage.quality;
			Sb2Player.app.stage.quality = StageQuality.LOW;
			bm.draw(dispObj, m);
			Sb2Player.app.stage.quality = oldQuality;
			Sb2Player.app.ignoreResize = false;
		}
