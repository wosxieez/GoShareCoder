...
//
// A com.goshare.Sb2Player sprite object. State specific to sprites includes: position, direction,
...
import com.goshare.interpreter.Variable;
...
public class ScratchSprite extends ScratchObj {
	...
	public function ScratchSprite(name:String = null) {
		objName = Sb2Player.app.stagePane.unusedSpriteName(name || Translator.map('Sprite1'));
	...
	public function setDirection(d:Number):void {
		...
		if(!Sb2Player.app.isIn3D) updateImage();
...
//	private var testSpr:Sprite = new Sprite();
	public function bitmap(forTest:Boolean = false):BitmapData {
		if (cachedBitmap != null && (!forTest || !Sb2Player.app.isIn3D))
			return cachedBitmap;
		...
		var b:Rectangle = (!Sb2Player.app.render3D || currentCostume().bitmap) ? img.getChildAt(0).getBounds(this) : getVisibleBounds(this);
...
//				updateEffectsFor3D();
//				var bm:BitmapData = com.goshare.Sb2Player.app.render3D.getRenderedChild(this, b.width * scaleX, b.height * scaleY);
...
//				}
	...
	public function handleTool(tool:String, evt:MouseEvent):void {
		...
		if (tool == 'help') Sb2Player.app.showTip('scratchUI');
	}

	private function growSprite():void { setSize(getSize() + 5); Sb2Player.app.updatePalette() }
	private function shrinkSprite():void { setSize(getSize() - 5); Sb2Player.app.updatePalette() }
...
//		app.libraryPart.showSpriteDetails(true);
	...
	private function saveToLocalFile():void {
		function success():void {
			Sb2Player.app.log(LogLevel.INFO, 'sprite saved to file', {filename: file.name});
		}
		var zipData:ByteArray = new ProjectIO(Sb2Player.app).encodeSpriteAsZipFile(copyToShare());
	...
	private function bubbleRect():Rectangle {
		...
		// compute bounds
		var p:Point = Sb2Player.app.stagePane.globalToLocal(localToGlobal(new Point(0, 0)));
		if (cachedBounds == null) bitmap(); // computes cached bounds
