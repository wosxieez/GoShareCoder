...
//
//	A reader for the serialized object format used to store com.goshare.Sb2Player projects (.sb file format).
//
...
//		a. read all objects
//		b. build image and com.goshare.sound objects
//		c. instantiate stage, sprite, costume, com.goshare.sound, and watcher objects
//		d. dereference object references (Ref objects) in the fields list of
//		   user-classes and in fixed-format collection objects.
//		e. initialize variable and list com.goshare.watchers
//		f. initialize costumes and sounds
//
//	Typically, the client calls readObjTable(), then scans the processed object table to
//	extract the stage, sprites, and com.goshare.watchers that constitute a com.goshare.Sb2Player project.
...
public class ObjReader {
	...
	public function readInfo():Object {
		...
		if (id != 'ScratchV01' && id != 'ScratchV02') {
			throw new IOError('Not a valid com.goshare.Sb2Player file');
		}
	...
	private function initSounds():void {
		...
		// To speed up reading old com.goshare.Sb2Player projects, compressed sounds
		// are kept in Squeak format until the project is saved.
		var cache:Dictionary = new Dictionary(); // used to avoid converting multiple identical copies of a com.goshare.sound to WAV
			...
			if (entry[1] == 164) { // SoundMedia
				...
				if (entry[9] == null) {
					...
					if (cache[sndData] != null) {
						snd.soundData = cache[sndData]; // use cached com.goshare.sound data
					} else {
