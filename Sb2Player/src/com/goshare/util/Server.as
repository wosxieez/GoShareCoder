...
//
// Interface to the com.goshare.Sb2Player website API's for Offline Editor.
//
...
public class Server implements IServer {
	...
	public function Server() {
		...
		try {
			var urlOverrides:String = Sb2Player.app.loaderInfo.parameters['urlOverrides'];
	...
	public function overrideURLs(overrides:Object):void {
		...
		var swfURL:String = Sb2Player.app.loaderInfo.url;
	...
	public function getOfficialExtensionURL(extensionName:String):String {
		...
		else if (Sb2Player.app.isExtensionDevMode) {
	...
	protected function onCallServerHttpStatus(url:String, data:*, event:HTTPStatusEvent):void {
		if (event.status < 200 || event.status > 299) {
			Sb2Player.app.logMessage(event.toString());
		}
	}
	...
	protected function onCallServerError(url:String, data:*, event:ErrorEvent):void {
		// Don't send this as an error since it seems to saturate our com.goshare.logging backend.
		Sb2Player.app.log(LogLevel.WARNING, 'Failed server request', {event: event, url: url, data: data});
		...
		if (event is SecurityErrorEvent) {
			...
			Sb2Player.app.log(LogLevel.WARNING, 'Reloading policy file', {policy: policyFileURL, initiator: url});
		}
	}
	...
	protected function onCallServerException(url:String, data:*, exception:*):void {
		if (exception is Error) {
			Sb2Player.app.logException(exception);
		}
	}
	...
	protected function callServer(url:String, data:*, mimeType:String, whenDone:Function,
		...
		if (data && url.indexOf('?') == -1) {
			url += '?v=' + Sb2Player.versionString + '&_rnd=' + Math.random();
			nextSeparator = '&';
		}
		...
		if (data) {
			...
			if (data.length == 0) {
				...
				// Most com.goshare.Sb2Player servers will respond to this with error 403, leading to other problems down the road.
	...
	protected function downloadThumbnail(url:String, w:int, h:int, whenDone:Function):URLLoader {
		function decodeImage(data:ByteArray):void {
			...
			try {
				...
				if (e is Error) {
					Sb2Player.app.logException(e);
				}
				else {
					Sb2Player.app.logMessage('Server caught exception decoding image: ' + url);
				}
			}
		}

		function imageError(e:IOErrorEvent):void {
			Sb2Player.app.log(LogLevel.WARNING, 'ServerOnline failed to decode image', {url: url});
		}
...
//		var url:String = getCdnStaticSiteURL() + 'medialibrarythumbnails/' + idAndExt;
	...
	public function getSelectedLang(whenDone:Function):void {
		...
		var sharedObj:SharedObject = SharedObject.getLocal('com.goshare.Sb2Player');
