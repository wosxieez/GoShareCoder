...
// Three types of projects are supported: old com.goshare.Sb2Player projects (.sb), new com.goshare.Sb2Player
// projects stored as a JSON project file and a collection of media files packed
// in a single ZIP file, and new com.goshare.Sb2Player projects stored on a server as a collection
...
import com.goshare.sound.WAVFile;
import com.goshare.sound.mp3.MP3Loader;

import com.goshare.svgutils.*;
...
public class ProjectIO {
	...
	public function ProjectIO(app:Sb2Player):void {
		this.app = app;
	}
	...
	private function addJSONData(fileName:String, obj:*, zip:ZipIO):void {
		...
		jsonData.writeUTFBytes(com.goshare.util.JSON.stringify(obj));
	...
	protected function decodeFromZipFile(zipData:ByteArray):ScratchObj {
		...
		var jsonObj:Object = com.goshare.util.JSON.parse(jsonData);
	...
	private function allImagesLoaded(objList:Array, imageDict:Dictionary, whenDone:Function, fail:Function):void {
		...
		if (errorDialog) {
			...
			errorDialog.showOnStage(Sb2Player.app.stage);
	...
	public function downloadProjectAssets(projectData:ByteArray):void {
			...
			if (app.lp) {
				...
				app.lp.setInfo(
						assetCount + ' ' +
								...
								Translator.map('com.goshare.assets loaded'));
			}
		...
		var projObject:Object = com.goshare.util.JSON.parse(projectData.readUTFBytes(projectData.length));
		var proj:ScratchStage = getScratchStage();
	...
	//----------------------------
	// Fetch a costume or com.goshare.sound from the server
	//----------------------------
	...
	public function fetchSound(id:String, sndName:String, whenDone:Function):void {
		// Fetch a com.goshare.sound asset from the server and call whenDone with the resulting ScratchSound.
	...
	public function fetchSprite(md5AndExt:String, whenDone:Function):void {
		...
		function jsonReceived(data:ByteArray):void {
			if (!data) return;
			spr.readJSON(com.goshare.util.JSON.parse(data.readUTFBytes(data.length)));
	...
	private function collectAssetsToFetch(objList:Array):Array {
		// Return list of MD5's for all project com.goshare.assets.
