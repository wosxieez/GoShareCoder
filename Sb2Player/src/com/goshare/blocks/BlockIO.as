...
//
// Convert com.goshare.blocks and stacks to/from an array structure or JSON string format.
...
public class BlockIO {
	...
	public static function stackToString(b:Block):String {
		return com.goshare.util.JSON.stringify(stackToArray(b));
	}

	public static function stringToStack(s:String, forStage:Boolean = false):Block {
		return arrayToStack(com.goshare.util.JSON.parse(s) as Array, forStage);
	}
		...
		for each (var a:* in b.normalizedArgs()) {
			...
			if (a is BlockArg) {
				...
				if (argVal is ScratchObj) {
					// convert a com.goshare.Sb2Player sprite/stage reference to a name string
	...
	private static function arrayToBlock(cmd:Array, undefinedBlockType:String, forStage:Boolean = false):Block {
		...
		if (cmd[0] == 'getUserName') Sb2Player.app.usesUserNameBlock = true;
		...
		for (var i:int = 0; i < args.length; i++) {
			var a:* = args[i];
			if (a is ScratchObj) {
				a = ScratchObj(a).objName; // convert a com.goshare.Sb2Player 1.4 sprite reference to a sprite name
				hadSpriteRef = true;
			}
			b.setArg(i, a);
		}
	...
	public static function specForCmd(cmd:Array, undefinedBlockType:String):Array {
		...
		var extensionSpec:Array = Sb2Player.app.extensionManager.specForCmd(op);
	...
	private static function argsForCmd(cmd:Array, numArgs:uint, reverseArgs:Boolean):Array {
		...
		// Arguments may be literal values or reporter com.goshare.blocks (expressions).
		var result:Array = [];
	...
	private static function specialCmd(cmd:Array, forStage:Boolean):Block {
		...
		case 'changeVariable':
			...
			return b;
		case 'EventHatMorph':
			if (cmd[1] == 'com.goshare.Sb2Player-StartClicked') {
	...
	private static function convertOldCmd(cmd:Array):Block {
		// If the given command is one of a handful of old com.goshare.Sb2Player com.goshare.blocks,
