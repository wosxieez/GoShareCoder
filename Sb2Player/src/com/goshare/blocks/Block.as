...
// or function (reporter). A stack is a sequence of command com.goshare.blocks, where
...
import com.goshare.scratch.*;

import com.goshare.translation.Translator;

import com.goshare.uiwidgets.*;
...
//	private static const blockLabelFormat:TextFormat = new TextFormat('LucidaBoldEmbedded', 10, 0xFFFFFF, true);
	...
	public var parameterNames:Array;	// used by procedure definition hats; null for other com.goshare.blocks
	...
	public var isTerminal:Boolean = false;	// com.goshare.blocks that end a stack like "stop" or "forever"
	...
	public function Block(spec:String, type:String = " ", color:int = 0xD00000, op:* = 0, defaultArgs:Array = null) {
		...
		if ((type == " ") || (type == "") || (type == "w")) {
			...
			isReporter = true;
			forceAsync = Sb2Player.app.extensionManager.shouldForceAsync(op);
			...
			isReporter = true;
			forceAsync = (type == 'r') && Sb2Player.app.extensionManager.shouldForceAsync(op);
			...
			isHat = true;
			forceAsync = (type == 'h') && Sb2Player.app.extensionManager.shouldForceAsync(op);
	...
	public function changeOperator(newOp:String):void {
		...
		// Note: This does not deal with com.goshare.translation, so it only works for symbolic operators.
	...
	private function runFeedbackFilters():Array {
		// com.goshare.filters for showing that a stack is running
	...
	public function restoreOriginalState():void {
		...
		case ROLE_ABSOLUTE:
			...
			break;
		case ROLE_EMBEDDED:
//			b.replaceArgWithBlock(b.args[originalIndex], this, Sb2Player.app.scriptsPane);
			break;
		case ROLE_NEXT:
			b.insertBlock(this);
			break;
	...
	public function fixArgLayout():void {
		...
		for (i = 0; i < labelsAndArgs.length; i++) {
			...
			// Next line moves the argument of if and if-else com.goshare.blocks right slightly:
		...
		if ([' ', '', 'o'].indexOf(type) >= 0) x = Math.max(x, minCommandWidth); // minimum width for command com.goshare.blocks
		if (['c', 'cf', 'e'].indexOf(type) >= 0) x = Math.max(x, minLoopWidth); // minimum width for C and E com.goshare.blocks
		if (['h'].indexOf(type) >= 0) x = Math.max(x, minHatWidth); // minimum width for hat com.goshare.blocks
	...
	public function removeBlock(b:Block):void {
		...
		SCRATCH::allow3d { Sb2Player.app.runtime.checkForGraphicEffects(); }
	}
	...
	public function showHelp():void {
		...
		if (extName) {
			if (Sb2Player.app.extensionManager.isInternal(extName)) {
				Sb2Player.app.showTip('ext:' + extName);
			}
			else {
				DialogBox.notify(
						'Help Missing',
	...
	public function duplicateStack(deltaX:Number, deltaY:Number):void {
		...
		var forStage:Boolean = Sb2Player.app.viewedObj() && Sb2Player.app.viewedObj().isStage;
		...
		Sb2Player.app.gh.grabOnMouseUp(newStack);
	}
	...
	public function deleteStack():Boolean {
		...
		// TODO: Remove any waiting reporter data in the com.goshare.Sb2Player.app.extensionManager
...
//		app.scriptsPane.saveScripts();
	...
	public function objToGrab(evt:MouseEvent):Block {
		if (isEmbeddedParameter() || isInPalette()) return duplicate(false, Sb2Player.app.viewedObj() is ScratchStage);
		return this;
	}
	...
	public function click(evt:MouseEvent):void {
		...
		Sb2Player.app.runtime.interp.toggleThread(topBlock(), Sb2Player.app.viewedObj(), 1);
	}
	...
	public function demo():void{
		...
		b.nextBlock = null;
		b.visible = false;
		Sb2Player.app.runtime.interp.toggleThread(b, Sb2Player.app.viewedObj(), 1);
	}
