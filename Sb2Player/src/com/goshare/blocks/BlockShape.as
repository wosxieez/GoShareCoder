...
public class BlockShape extends Shape {
	...
	public function setWidthAndTopHeight(newW:int, newTopH:int, doRedraw:Boolean = false):void {
		...
		// and reporter com.goshare.blocks, the top height is the height of the block.
		// For C and E shaped com.goshare.blocks (conditionals and loops), the top height
	...
	private function blockShapeFilters():Array {
		// com.goshare.filters for command and reporter Block outlines
	...
	private function dropFeedbackFilters(forReporter:Boolean):Array {
		// com.goshare.filters for command/reporter block drop feedback
		var f:GlowFilter;
		if (forReporter) {
	...
	private function drawLoopArrow(g:Graphics, h:int):void {
		// Draw the arrow on loop com.goshare.blocks.
		var arrow:Array = [
			[8, 0], [2, -2], [0, -3],
			[3, 0], [-4, -5], [-4, 5], [3, 0],
			[0, 3], [-8, 0], [0, 2]];
