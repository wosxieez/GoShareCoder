...
public class Log {
	...
	public function log(severity:String, messageKey:String, extraData:Object = null):LogEntry {
		...
		function getExtraString():String {
			if (!extraString) {
				try {
					extraString = com.goshare.util.JSON.stringify(extraData);
				} catch (e:*) {
		...
		if (Sb2Player.app.jsEnabled) {
			if (echoToJS) {
				if (extraData) {
					Sb2Player.app.externalCall('console.log', null, getEntryString(), extraData);
				}
				else {
					Sb2Player.app.externalCall('console.log', null, getEntryString());
				}
			}
			if (LogLevel.TRACK == severity) {
				Sb2Player.app.externalCall('JStrackEvent', null, messageKey, extraData ? getExtraString() : null);
			}
		}
		return entry;
	}
