...
// transformed, leaving no remaining transform. For text and image elements, the com.goshare.translation part
...
public class SVGImporter {
	...
	public function SVGImporter(svg:XML) {
		...
		if (!root) {
			Sb2Player.app.log(LogLevel.WARNING, 'No SVG root (empty file?)');
	...
	private function extractElement(xml:XML, parentList:Array):SVGElement {
		...
		// or null if the given element is empty or is ignored by com.goshare.Sb2Player.
		// Recursively extract subelements if appropriate.
		var tag:String = xml.localName();
		if (ignoredTags.indexOf(tag) >= 0) return null; // ignored by com.goshare.Sb2Player
		switch (tag) {
		case 'circle':
		case 'clipPath':
		case 'ellipse':
	...
	private function readSwitch(xml:XML, parentList:Array):SVGElement {
		...
		// only child is the first switch element that can be handled by com.goshare.Sb2Player.
	...
	private function computeTransforms(el:SVGElement, inherited:Matrix):void {
		...
		if (false && el.transform) { // transform prebaking code; no longer used but could be useful in the future
			...
			if (isTranslationMatrix(el.transform)) {
				if ('image' == el.tag) {
					// Simple com.goshare.translation; just move the element and discard the transform.
