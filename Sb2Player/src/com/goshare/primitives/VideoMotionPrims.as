...
//
// Video motion sensing com.goshare.primitives.
...
public class VideoMotionPrims {
	...
	public function VideoMotionPrims(app:Sb2Player, interpreter:Interpreter) {
		this.app = app;
	...
	private function getLocalMotion(s:ScratchSprite):void {
		...
		if (s.localFrameNum != frameNum) {
			...
			if (s.localMotionAmount > (THRESHOLD/3)) {
				s.localMotionDirection = ((Math.atan2(v, u) * toDegree + 270) % 360) - 180; // com.goshare.Sb2Player direction
			}
	...
	private function analyzeFrame():void {
		...
		if (motionAmount > THRESHOLD) {
			motionDirection = ((Math.atan2(vv, uu) * toDegree + 270) % 360) - 180; // com.goshare.Sb2Player direction
		}
		analysisDone = true;
	}
