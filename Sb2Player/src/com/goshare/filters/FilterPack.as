...
//
// com.goshare.Sb2Player image com.goshare.filters. Uses compiled "pixel-bender" shaders for performance.
// Use setFilter() to set filter parameters. buildFilters() returns a list of com.goshare.filters
// to be assigned to the com.goshare.filters property of s DisplayObject (e.g. a sprite).

package com.goshare.filters {
...
public class FilterPack {
	...
	public function buildFilters(force:Boolean = false):Array {
		// disable com.goshare.filters not running on x86 because PixelBender is really slow
		if((Sb2Player.app.isIn3D || Capabilities.cpuArchitecture != 'x86') && !force) return emptyArray;

		var scale:Number = targetObj.isStage ? 1 : Sb2Player.app.stagePane.scaleX;
		...
		if (filterDict["pixelate"] != 0) {
			...
			if (targetObj == Sb2Player.app.stagePane) n *= Sb2Player.app.stagePane.scaleX;
