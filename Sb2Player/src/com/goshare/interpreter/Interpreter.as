...
// Revised, March 2010
//
// A simple yet efficient com.goshare.interpreter for com.goshare.blocks.
//
// Interpreters may seem mysterious, but this one is quite straightforward. Since every
// block knows which block (if any) follows it in a sequence of com.goshare.blocks, the com.goshare.interpreter
// simply executes the current block, then asks that block for the next block. The heart
// of the com.goshare.interpreter is the evalCmd() function, which looks up the opcode string in a
...
//
// The com.goshare.interpreter does as much as it can within workTime milliseconds, then returns
...
//
// To add a command to the com.goshare.interpreter, just add a new case to initPrims(). Command com.goshare.blocks
...
// a 32-bit integer, it wraps after 24.86 days. Since it seems unlikely that one com.goshare.Sb2Player
// session would run that long, this code doesn't deal with clock wrapping.
// Since com.goshare.Sb2Player only runs at discrete intervals, timed commands may be resumed a few
...
package com.goshare.interpreter {
...
public class Interpreter {
	...
	public function Interpreter(app:Sb2Player) {
		this.app = app;
		initPrims();
//		checkPrims();
	}
	...
	private function initPrims():void {
        ...
        primTable["doIfGoShare"]		= function(b:*):* { startCmdList(b.subStack1); };
