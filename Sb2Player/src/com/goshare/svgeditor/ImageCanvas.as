...
public class ImageCanvas extends Sprite {
	...
	public function getBackDropFills():Array {
		...
		for (var i:uint = 0; i < contentLayer.numChildren; ++i) {
			...
			if (dObj is SVGShape) {
				var st:String = (dObj as SVGShape).getElement().getAttribute('com.goshare.scratch-type');
				if (st == 'backdrop-fill')
					fills.push(dObj);
			}
		}
	...
	public function addBackdropStroke(s:SVGShape):void {
		for (var i:uint = 0; i < contentLayer.numChildren; ++i) {
			var dObj:DisplayObject = contentLayer.getChildAt(i);
			if (!(dObj is SVGShape) ||
					(dObj as SVGShape).getElement().getAttribute('com.goshare.scratch-type') != 'backdrop-fill') {
	...
	public function handleTool(tool:String, evt:MouseEvent):void {
		if ('help' == tool) Sb2Player.app.showTip('paint');
