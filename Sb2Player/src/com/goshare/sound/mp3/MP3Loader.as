...
public class MP3Loader {
	...
	public static function convertToScratchSound(sndName:String, sndData:ByteArray, whenDone:Function):void {
		...
		// Call whenDone with the new com.goshare.sound if successful.
		...
		if (mp3Info.sampleCount == 0) { // bad MP3 data
			if (Sb2Player.app.lp) Sb2Player.app.removeLoadProgressBox();
			whenDone(null);
			return;
		}
		load(sndData, loaded);
	...
	public static function extractSamples(sndName:String, mp3Snd:Sound, mp3SampleCount:int, whenDone:Function):void {
		...
		// Note: The Flash extract() method provides com.goshare.sound data as stereo at 44100 samples/second
		...
		function convertNextChunk():void {
			...
			if (count == 0 || convertedSamples >= mp3SampleCount) { // finished!
				if (Sb2Player.app.lp) Sb2Player.app.lp.setTitle('Compressing...');
			...
			if (Sb2Player.app.lp) Sb2Player.app.lp.setProgress(Math.min(convertedSamples / mp3SampleCount, 1));
