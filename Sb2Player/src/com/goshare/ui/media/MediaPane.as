...
public class MediaPane extends ScrollFrameContents {
	...
	public function MediaPane(app:Sb2Player, type:String):void {
		this.app = app;
		isSound = (type == 'sounds');
		refresh();
	}
	...
	private function costumeItems():Array {
		...
		for each (var c:ScratchCostume in viewedObj.costumes) {
			result.push(Sb2Player.app.createMediaInfo(c, viewedObj));
		}
		return result;
	}
	...
	private function soundItems():Array {
		...
		for each (var snd:ScratchSound in viewedObj.sounds) {
			result.push(Sb2Player.app.createMediaInfo(snd, viewedObj));
		}
		return result;
	}
	...
	private function changeMediaOrder(dropped:MediaInfo):void {
		...
		// update the target object with the new costume/com.goshare.sound list
		// refresh();
	}
	...
	private function replacedMedia(newList:Array):void {
		// Note: Clones can share the costume and com.goshare.sound arrays with their prototype,
