...
import com.goshare.extensions.ScratchExtension;
import com.goshare.scratch.*;
import com.goshare.sound.mp3.MP3Loader;
...
public class MediaLibrary extends Sprite {
	...
	public function MediaLibrary(app:Sb2Player, type:String, whenDone:Function) {
		this.app = app;
		this.assetType = type;
	...
	public static function strings():Array {
		var result:Array = [
			...
			'Uploading image...', 'Uploading sprite...', 'Uploading com.goshare.sound...',
			'Importing com.goshare.sound...', 'Converting mp3...',
		];
...
//		app.mediaLibrary = this;
	...
	public function importFromDisk():void {
		if (parent) close();
		if (assetType == 'com.goshare.sound') importSoundsFromDisk();
...
//		app.mediaLibrary = null;
	...
	private function addTitle():void {
		...
		if ('com.goshare.sound' == s) s = 'Sound Library';
	...
	private function addFilters():void {
		...
		if ('com.goshare.sound' == assetType) categories = soundCategories;
...
spriteFeaturesFilter.visible = false; // disable features filter for now
	...
	private function filterChanged(filter:MediaFilter):void {
		...
		// scroll to top when com.goshare.filters change
		resultsPane.y = 0;
	...
	private function addResultsFrame():void {
		...
		resultsPane.backgroundColor = CSS.white;
	...
	private function viewLibrary():void {
		function gotLibraryData(data:ByteArray):void {
			...
			libraryCache[assetType] = com.goshare.util.JSON.parse(stripComments(s)) as Array;
			collectEntries();
		}
			...
			for each (var entry:Object in libraryCache[assetType]) {
				if (entry.type == assetType) {
					...
					if (info) {
						...
						if (entry.type == 'com.goshare.sound') {
	...
	public function addSelected():void {
		...
		for (var i:int = 0; i < resultsPane.numChildren; i++) {
			...
			if (item && item.isHighlighted()) {
				...
				} else if (assetType == 'com.goshare.sound') {
	...
	private function convertAndUploadImageOrSprite(fName:String, data:ByteArray):void {
		...
		function decodeError():void {
			DialogBox.notify('Error decoding image', 'Sorry, com.goshare.Sb2Player was unable to load the image '+fName+'.', Sb2Player.app.stage);
		}
		function spriteError():void {
			DialogBox.notify('Error decoding sprite', 'Sorry, com.goshare.Sb2Player was unable to load the sprite '+fName+'.', Sb2Player.app.stage);
		}
		var costumeOrSprite:*;
		var fExt:String = '';
	...
	protected function startSoundUpload(sndToUpload:ScratchSound, origName:String, whenDone:Function):void {
		if(!sndToUpload) {
			DialogBox.notify(
					'Sorry!',
					'The com.goshare.sound file '+origName+' is not recognized by com.goshare.Sb2Player.  Please use MP3 or WAV com.goshare.sound files.',
					stage);
			return;
		}
		whenDone();
	...
	private function convertAndUploadSound(sndName:String, data:ByteArray):void {
		...
		app.addLoadProgressBox('Importing com.goshare.sound...');
		try {
		...
		if (snd && snd.sampleCount > 0) { // WAV data
			...
			function uploadConvertedSound(out:ScratchSound):void {
				...
				if (snd && snd.sampleCount > 0) {
					...
					app.removeLoadProgressBox();
					DialogBox.notify('Error decoding com.goshare.sound', 'Sorry, com.goshare.Sb2Player was unable to load the com.goshare.sound ' + sndName + '.', Sb2Player.app.stage);
				}
			}
			SCRATCH::allow3d {
				sound = new Sound();
