...
//
// This object represents an image, com.goshare.sound, or sprite in the MediaLibrary. It displays
...
package com.goshare.ui.media {
	...
	import com.goshare.sound.mp3.MP3SoundPlayer;
	import com.goshare.svgutils.SVGImporter;
...
public class MediaLibraryItem extends Sprite {
	...
	private function setImageThumbnail(md5:String, done:Function, spriteMD5:String = null):void {
		...
		if (fileType(md5) == 'svg') loaders.push(Sb2Player.app.server.getAsset(md5, gotSVGData));
		else loaders.push(Sb2Player.app.server.getThumbnail(md5, thumbnailWidth, thumbnailHeight, setThumbnail));
	}
	...
	private function setSpriteThumbnail(done:Function):void {
		function gotJSONData(data:String):void {
			var md5:String;
			if (data) {
				var sprObj:Object = com.goshare.util.JSON.parse(data);
		...
		else loaders.push(Sb2Player.app.server.getAsset(spriteMD5, gotJSONData));
	}
	...
	private function downloadAndPlay():void {
		// Download and play a library com.goshare.sound.
		function gotSoundData(wavData:ByteArray):void {
			if (!wavData) return;
			sndData = wavData;
			startPlayingSound();
		}
		Sb2Player.app.server.getAsset(dbObj.md5, gotSoundData);
	}
