...
//
// This object represent a com.goshare.sound, image, or script. It is used:
...
public class MediaInfo extends Sprite {
	...
	public var owner:ScratchObj; // object owning a com.goshare.sound or costume in MediaPane; null for other cases
	...
	private function backpackTitle():String {
		...
		if ('com.goshare.sound' == objType) return Translator.map('Sound');
	...
	public function objToGrab(evt:MouseEvent):* {
		var result:MediaInfo = Sb2Player.app.createMediaInfo({
			type: objType,
			name: objName,
			width: objWidth,
			md5: md5
		});
		if (mycostume) result = Sb2Player.app.createMediaInfo(mycostume, owner);
		if (mysound) result = Sb2Player.app.createMediaInfo(mysound, owner);
		if (mysprite) result = Sb2Player.app.createMediaInfo(mysprite);
		if (scripts) result = Sb2Player.app.createMediaInfo(scripts);
	...
	protected function duplicateMe():void {
		if (owner && !getBackpack()) {
			if (mycostume) Sb2Player.app.addCostume(mycostume.duplicate());
			if (mysound) Sb2Player.app.addSound(mysound.duplicate());
		}
	}

	protected function deleteMe(ignore:* = null):void {
		if (owner) {
			Sb2Player.app.runtime.recordForUndelete(this, 0, 0, 0, owner);
			if (mycostume) {
				owner.deleteCostume(mycostume);
				Sb2Player.app.refreshImageTab(false);
			}
			if (mysound) {
				owner.deleteSound(mysound);
				Sb2Player.app.refreshSoundTab();
			}
		}
	}
