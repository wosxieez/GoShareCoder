...
//
// com.goshare.Sb2Player extension gpip. Maintains a dictionary of all com.goshare.extensions in use and manages
// socket-based communications with local and server-based extension helper applications.

package com.goshare.extensions {
    import com.goshare.Sb2Player;
    import com.goshare.Specs;
...
import com.goshare.interpreter.*;

import mx.utils.URLUtil;

import com.goshare.uiwidgets.DialogBox;
import com.goshare.uiwidgets.IndicatorLight;

import com.goshare.util.*;

public class ExtensionManager {

	protected var app:Sb2Player;
	...
	// Experimental com.goshare.extensions must be hosted on one of these domains
	...
	public function ExtensionManager(app:Sb2Player) {
		this.app = app;
	...
	public function clearImportedExtensions():void {
		...
		// Clear imported com.goshare.extensions before loading a new project.
		extensionDict = {};
	...
	// Should the com.goshare.interpreter force async communication with com.goshare.extensions?
	...
	public function shouldForceAsync(op:String):Boolean {
		...
		if (extension && extension.port != 0) {
			// HTTP com.goshare.extensions should never be forceAsync
			return false;
		}
		else {
			// JS com.goshare.extensions should be forceAsync in the offline editor.
	...
	// Retrieve all specs from all loaded com.goshare.extensions. Used by the com.goshare.translation system.
	// Remember to load all relevant com.goshare.extensions before exporting com.goshare.translation strings!
	public function getExtensionSpecs(warnIfMissing:Boolean):Array {
		...
		if (warnIfMissing && missingExtensions.length > 0) {
			DialogBox.notify(
					'Missing block specs', 'Block specs were missing for some com.goshare.extensions.\n' +
					'Please load these com.goshare.extensions and try again:\n' + missingExtensions.join(', '));
		}
		return specs;
	}
	...
	public function setEnabled(extName:String, flag:Boolean):void {
		...
		if (ext && ext.showBlocks != flag) {
			...
			if(app.jsEnabled && ext.javascriptURL) {
				if(flag) {
					var javascriptURL:String = ext.isInternal ? Sb2Player.app.fixExtensionURL(ext.javascriptURL) : ext.javascriptURL;
	...
	public function enabledExtensions():Array {
		// Answer an array of enabled com.goshare.extensions, sorted alphabetically.
		var result:Array = [];
		for each (var ext:ScratchExtension in extensionDict) {
	...
	public function stopButtonPressed():* {
		// Send a reset_all command to all active com.goshare.extensions.
	...
	public function extensionsToSave():Array {
		// Answer an array of extension descriptor objects for imported com.goshare.extensions to be saved with the project.
		var result:Array = [];
		for each (var ext:ScratchExtension in extensionDict) {
	...
	public function loadRawExtension(extObj:Object):ScratchExtension {
		...
		Sb2Player.app.translationChanged();
		Sb2Player.app.updatePalette();
...
//		}
	...
	public function loadSavedExtensions(savedExtensions:Array):void {
		function extensionRefused(extObj:Object, reason: String):void {
			Sb2Player.app.jsThrowError('Refusing to load project extension "' + extObj.extensionName + '": ' + reason);
		}

		// Reset the system com.goshare.extensions and load the given array of saved com.goshare.extensions.
		if (!savedExtensions) return; // no saved com.goshare.extensions
		for each (var extObj:Object in savedExtensions) {
			...
			if (!('extensionName' in extObj)) {
				Sb2Player.app.jsThrowError('Refusing to load project extension without a name.');
				continue;
			}
			...
			if(extObj.javascriptURL) {
				if (!Sb2Player.app.isExtensionDevMode) {
					extensionRefused(extObj, 'Experimental com.goshare.extensions are only supported on ScratchX.');
					continue;
				}
				...
				if (!domainAllowed) {
					extensionRefused(
							extObj,
							'Experimental com.goshare.extensions must be hosted on an approved domain. Approved domains are: ' +
							allowedDomains.join(', '));
					continue;
				}
				ext.javascriptURL = extObj.javascriptURL;
		...
		if(ext.port > 0) {
			...
			function statusCallback(retval:Object):void {
				...
				if(retval.status == 2) color = 0x00C000;
					...
					color = 0xE00000;
					if(firstTime) {
						Sb2Player.app.showTip('com.goshare.extensions');
//					DialogBox.notify('Extension Problem', 'It looks like the '+ext.name+' is not working properly.' +
//							'Please read the com.goshare.extensions help in the tips window.', com.goshare.Sb2Player.app.stage);
	...
	public function call(extensionName:String, op:String, args:Array):void {
		...
		if (ext.port > 0) {
			...
			app.interp.redraw(); // make sure com.goshare.interpreter doesn't do too many extension calls in one cycle
		}
	}
	...
	public function step():void {
		// Poll all com.goshare.extensions.
		for each (var ext:ScratchExtension in extensionDict) {
			if (ext.showBlocks) {
	...
	private function httpGetSpecs(ext:ScratchExtension):void {
		...
		function completeHandler(e:Event):void {
			var specsObj:Object;
			try {
				specsObj = com.goshare.util.JSON.parse(loader.data);
			} catch(e:*) {}
			if (!specsObj) return;
